...
class RenderService:
    ...
    def __init__(self, *, s3: S3Client, cache: RenderCache) -> None:
        self._s3 = s3
        self._cache = cache
        # Parallel dict keyed identically to the cache, holding metadata so
        # cache hits can return the same envelope as fresh renders.
        self._meta: dict[tuple[str, str], RenderMetadata] = {}
    ...
    def render(self, req: RenderRequest) -> RenderResponse:
        ...
        if cached_png is not None:
            return RenderResponse(
                png=cached_png,
                metadata=self._meta[(req.station, cache_key)],
            )
